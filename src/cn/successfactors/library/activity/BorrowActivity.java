...
//        String url1 = "http://192.168.25.222:8089/SFLibraryService/private/borrow/getborrowlistpage/当前记录/" + username + "/20/1";
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(final Message msg) {
            ...
            Gson gson = new Gson();
            BorrowPage borrowPage = gson.fromJson(bundleResult, BorrowPage.class);
    ...
    public class MyAdapter extends BaseAdapter{
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                ...
                convertView = mInflater.inflate(R.layout.borrow_book_item, null);
                holder.name = (TextView)convertView.findViewById(R.id.borrowBookName);
                holder.author = (TextView)convertView.findViewById(R.id.borrowBookAuthor);
                holder.image = (ImageView)convertView.findViewById(R.id.borrowBookImage);
                holder.borrowDate = (TextView)convertView.findViewById(R.id.borrowStartDate);
                holder.dueDate = (TextView)convertView.findViewById(R.id.borrowDueDate);
            ...
            holder.borrowDate.setText(getResources().getString(R.string.borrow_date) + " " + borrow.getBorrowDate().substring(0, 10));
            holder.dueDate.setText(getResources().getString(R.string.due_date) + " " + borrow.getShouldReturnDate().substring(0, 10));
		...
		if((firstVisibleItem+visibleItemCount ==totalItemCount) && totalItemCount!=0) {
            if(!isLoading) {
                if(page < totalPage) {
                	SharedPreferences userInfo = getSharedPreferences(Constants.USERPREFERENCE, 0);
                    String username = userInfo.getString(Constants.USERNAME, ""); 
                    String url = Constants.ROOT_PATH + SUB_PATH + username + "/" + pageSize + "/" + ++page; 
//                    performRequest("http://192.168.25.222:8089/SFLibraryService/private/borrow/getborrowlistpage/当前记录/" + username + "/20/" + ++page);
                    performRequest(url);
                } else {
	...
	private void loadData(){
		...
		page = 1;
		SharedPreferences userInfo = getSharedPreferences(Constants.USERPREFERENCE, 0);  
        performRequest(Constants.ROOT_PATH + SUB_PATH + userInfo.getString(Constants.USERNAME, null) + "/" + pageSize + "/1");
	}
}
