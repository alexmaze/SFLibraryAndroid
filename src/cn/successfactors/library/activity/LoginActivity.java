...
public class LoginActivity extends Activity {
	...
	OnClickListener loginListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			imm.hideSoftInputFromWindow(
					((EditText) findViewById(R.id.LoginID)).getWindowToken(), 0);

			// username and password both can not be empty
			userName = ((TextView) findViewById(R.id.LoginID)).getText()
					.toString();
	...
	public void doLogin() {
		...
		try {
			...
			if (result.optString("sessionKey") == null
					|| result.isNull("sessionKey")) {
				msg.what = LOGIN_FAILED;
			} else {
				msg.what = LOGIN_SUCCESS;

				userInfo.edit()
						.putString(Constants.SESSIONKEY,
								result.getString("sessionKey")).commit();
				userInfo.edit().putString(Constants.USERNAME, userName)
						.commit();
				if (cbPassword.isChecked()) {
					userInfo.edit().putString(Constants.PASSWORD, password)
							.commit();
				} else {
